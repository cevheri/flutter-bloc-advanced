...
class UserBloc extends Bloc<UserEvent, UserState> {
  ...
  FutureOr<void> _onEditorInit(
    UserEditorInit event,
  ...
  FutureOr<void> _onSubmit(
    ...
    _log.debug("BEGIN: onSubmit UserSubmitEvent event: {}", [
      event.user.toString(),
    ]);
    emit(state.copyWith(status: UserStatus.loading));
    try {
      final user = event.user.id == null
          ? await _repository.create(event.user)
          : await _repository.update(event.user);
      emit(state.copyWith(status: UserStatus.saveSuccess, data: user));
      _log.debug("END:onSubmit UserSubmitEvent event success: {}", [
        user.toString(),
      ]);
    } catch (e) {
      emit(state.copyWith(status: UserStatus.failure));
      _log.error("END:onSubmit UserSubmitEvent event error: {}", [
        e.toString(),
      ]);
    }
  }
  ...
  FutureOr<void> _onDelete(
    ...
    try {
      if (event.id == "user-1") {
        ...
        _log.error("END:onDelete UserDelete event error: {}", [
  ...
  FutureOr<void> _onFetchUser(
    ...
    try {
      ...
      _log.debug("END:onFetchUser FetchUserEvent event success: {}", [
        entity.toString(),
      ]);
    } catch (e) {
      emit(state.copyWith(status: UserStatus.failure, err: e.toString()));
      _log.error("END:onFetchUser FetchUserEvent event error: {}", [
        e.toString(),
      ]);
    }
  }
  ...
  FutureOr<void> _onSearch(
    ...
    _log.debug("BEGIN: onSearch UserSearch event. name:{} authority: {}", [
      event.name,
      event.authorities,
    ]);
    emit(state.copyWith(status: UserStatus.loading));
    try {
      if ((event.name == null || event.name == "") &&
          (event.authorities == null || event.authorities == "")) {
        final entities = await _repository.list();
        emit(
          state.copyWith(status: UserStatus.searchSuccess, userList: entities),
        );
        _log.debug(
          "END:onSearch UserSearch event success - list. content count: {}",
          [entities.length],
        );
        return;
        ...
        final entities = await _repository.listByNameAndRole(
          event.page,
          event.size,
          event.name!,
          event.authorities!,
        ...
        _log.debug(
          "END:onSearch UserSearch event with name success - name and authority content count: {}",
          [entities.length],
        );
        return;
      } else if (event.authorities != null && event.authorities!.isNotEmpty) {
        final entities = await _repository.listByAuthority(
          event.page,
          event.size,
          event.authorities!,
        );
        ...
        _log.debug(
          "END:onSearch UserSearch event success authority - content count: {}",
          [entities.length],
        );
        return;
      }
