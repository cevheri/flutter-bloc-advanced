...
void main() {
  ...
  group("Login Repository authenticate", () {
    ...
    test(
      "Given valid entity when authenticate then return JWTToken successfully",
      () async {
    ...
    test(
      "Given valid entity without AccessToken when authenticate then return JWTToken fail",
      () async {
        const entity = mockUserJWTPayload;
        final result = await LoginRepository().authenticate(entity);
    ...
    test(
      "Given null entity when authenticate then return JWTToken fail",
      () async {
        expect(
          () async =>
              await LoginRepository().authenticate(const UserJWT("", "")),
    ...
    test(
      "Given stored entity when logout then clear storage successfully",
      () async {
        TestUtils().setupAuthentication();

        expect(
          await AppLocalStorage().read(StorageKeys.jwtToken.name),
          isNotNull,
        );
        expect(
          await AppLocalStorage().read(StorageKeys.jwtToken.name),
  ...
  group("Login Repository sendOtp", () {
    test("Given valid email when sendOtp then complete successfully", () async {
      ...
      expect(
        () async => await LoginRepository().sendOtp(request),
        returnsNormally,
      );
    });

    test(
      "Given invalid email when sendOtp then throws BadRequestException",
      () async {
        TestUtils().setupAuthentication();
        final request = SendOtpRequest(email: "");

        await expectLater(
          LoginRepository().sendOtp(request),
  ...
  group("Login Repository verifyOtp", () {
    test(
      "Given valid OTP when verify then return JWTToken successfully",
      () async {
        TestUtils().setupAuthentication();
    ...
    test(
      "Given invalid OTP when verify then throws BadRequestException",
      () async {
        TestUtils().setupAuthentication();
        final request = VerifyOtpRequest(
          email: "test@example.com",
        ...
        await expectLater(
          LoginRepository().verifyOtp(request),
          throwsA(isA<BadRequestException>()),
        );
      },
    );
