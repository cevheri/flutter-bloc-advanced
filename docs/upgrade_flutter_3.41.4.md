...
### build_runner No Longer Needed

The command `dart run build_runner build --delete-conflicting-outputs` is **no longer required**. It was used to generate `.mapper.g.dart` files for `dart_json_mapper` + `reflectable`. Since these packages are removed, `build_runner` is also removed from dependencies.

**Note about mockito:** The existing `.mocks.dart` files (generated by `@GenerateMocks` / `@GenerateNiceMocks`) are committed to version control and work as-is. If you need to regenerate them in the future (e.g., when adding new mock classes), you would need to temporarily add `build_runner` back as a dev dependency.
